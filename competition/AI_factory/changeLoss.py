...
class TverskyLoss(Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        tversky_loss = 1 - tf.reduce_mean(tversky)
...
# 저장 이름
save_name = 'base_line_changeLoss'
...
# 중간 가중치 저장 이름
CHECKPOINT_PERIOD = 5
CHECKPOINT_MODEL_NAME = 'checkpoint-{}-{}-epoch_{{epoch:02d}}BASE2.hdf5'.format(MODEL_NAME, save_name)
 
# 최종 가중치 저장 이름
FINAL_WEIGHTS_OUTPUT = 'model_{}_{}_final_weightsBASE2.h5'.format(MODEL_NAME, save_name)
...
model.compile(optimizer = Adam(learning_rate=learning_rate), loss =TverskyLoss(alpha=0.3, beta=0.7), metrics = ['accuracy', miou])
model.summary()
...
# model.load_weights('C:\\_data\\dataset\\output\\model_unet_hoon5_line_final_weights.h5')
...
# joblib.dump(y_pred_dict, 'C:\\_data\\dataset\\output\\y_pred5.pkl')